...
from tradebot.domain.ports import SignalParserPort, TradingEnginePort
...
class TelegramSignalListener:
    ...
    def __init__(self,
        ...
        self.parser = parser
        self.engine = engine
        self.generator = order_generator or OrderGenerator()
    ...
    async def _handle(self, upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        orders: list[Order] = self.generator.generate(sig)
        if not orders:
        ...
        for order in orders:
            ...
            responses.append(f"{order.side.upper()} {order.volume} {order.symbol} -> TP {order.tp} : {status}")
